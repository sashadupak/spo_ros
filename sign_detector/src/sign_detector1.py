...
class SignDetector:
    ...
    def __init__(self, ref_folder):
        ...
        self.min_match = 50 # min number of matches
    ...
    def classify_sign(self, sign):
        ...
        if len(matches_list) > 0:
            ...
            sign_type = ref_name.split("\\")[-1]

            #if len(matches) > self.min_match::
            sign, corners = self.transform(sign, matches, kp1, self.kp2[ref_name])
        else:
            matches = []
            sign_type = "UNDEFINED"
        return sign, sign_type, len(matches)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for file_name in file_names:
            ...
            cv2.imshow('img', sign)
            #if sign_found:
            sign, sign_type, num_matches = detector.classify_sign(img)
